/**
 * POST /api/promo/codes/update — met à jour un code (admin)
 */
import { NextRequest, NextResponse } from 'next/server'
import { requireAdmin, ApiError } from '@/lib/api-auth'
import { updatePromoCode } from '@/lib/db-promo-access'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const out = await updatePromoCode({ id, code, duration_days: Number.isFinite(duration_days) ? duration_days : null, max_uses: Number.isFinite(max_uses) ? max_uses : null, expires_at, note })
