...
export async function POST(req: NextRequest) {
  try {
    ...
    let coach = COACH_SYSTEM_PROMPT
    let fromFile = false
    for (const p of OVERRIDES_PATHS) {
      try {
        const raw = await readFile(p, 'utf8')
        const j = JSON.parse(raw) as { tuteur?: string; threshold?: string; coach?: string }
        if (j.tuteur) {
          tuteur = j.tuteur
          fromFile = true
        }
    ...
    const nameCoach = fromFile ? 'Importé depuis overrides (Coach)' : 'Par défaut (Coach)'
    const ids = await importContent(tuteur, threshold, coach, nameTuteur, nameThreshold, nameCoach)
