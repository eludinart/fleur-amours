...
import { replaceAuthCookie, setAdminBackupCookie } from '@/lib/auth-cookie'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const [adminUser, user] = await Promise.all([
      authMe(parseInt(adminUserId, 10)),
      authMe(parseInt(String(targetUserId), 10)),
    ])
    ...
    const res = NextResponse.json({
      token,
      user,
      admin_backup_token: currentToken || null,
      admin_user: adminUser,
    })
    if (currentToken) {
      setAdminBackupCookie(res, currentToken)
    }
    replaceAuthCookie(res, token)
    return res
  } catch (err: unknown) {
