...
function CoachDirectoryCard({ coach: c }: { coach: Coach }) {
  ...
  return (
    <div
      className={`rounded-2xl border transition-colors bg-white dark:bg-slate-900/80 shadow-sm overflow-hidden ${
        open
        ...
        <div className="min-w-0 flex-1 space-y-1 text-left">
          <div className="flex flex-wrap items-center gap-2 gap-y-1">
            ...
            <span className="ml-auto text-violet-600 dark:text-violet-300 text-xs font-semibold shrink-0">
              {open ? t('coaches.closeFiche') : t('coaches.openFiche')}
              <span className={`inline-block transition-transform ml-0.5 ${open ? 'rotate-180' : ''}`}>▾</span>
            </span>
          </div>
          {handle && (c.name || '').trim() ? (
