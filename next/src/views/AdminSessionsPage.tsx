...
function SessionDetailModal({
  ...
  return (
    <div
      className="fixed inset-0 bg-black/90 backdrop-blur-sm z-40 flex items-start justify-center pt-8 px-4 overflow-y-auto"
      onClick={onClose}
    >
      <div
          ...
          {tab === 'plan' && (
            <div className="space-y-5">
              {!session.plan14j ? (
                ...
                <>
                  {(session.plan14j.synthesis ||
                    session.plan14j.synthesis_suggestion) && (
                      ...
                      {[0, 1].map((w) => (
                        <div key={w}>
                          ...
                          <div className="space-y-1.5">
                            {session
                              .plan14j!.plan_14j!.slice(w * 7, w * 7 + 7)
                              .map((day, idx) => (
                                <div
                                  key={day.day ?? idx}
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">
                      {shadowEvents.length} détection
                      {shadowEvents.length > 1 ? 's' : ''} au cours de la session
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-violet-200 dark:border-violet-800 bg-violet-50/30 dark:bg-violet-950/10 p-4">
                ...
                <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
                  Ces détections signalent des zones de vulnérabilité ou de tension
                  exprimées au cours de la session. Un contact bienveillant peut être
                  pertinent pour proposer un accompagnement adapté.
                </p>
              </div>
            </div>
          )}
...
export default function AdminSessionsPage() {
  ...
  const [shadowOnly, setShadowOnly] = useState(
  ...
  useEffect(() => {
    sessionsApi.stats().then(setStats as (d: unknown) => void).catch(() => {})
  }, [])
  ...
  const loadList = useCallback(() => {
    sessionsApi
      ...
      .then(setSessions as (d: unknown) => void)
      .catch(() => {})
  }, [page, search, dateFrom, dateTo, statusFilter, shadowOnly])
  ...
  async function openDetail(id: string) {
    ...
    try {
      const full = (await sessionsApi.get(id)) as Session
      setSelected(full)
    } catch {
      /* ignore */
    } finally {
    ...
    <div className="space-y-6">
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          <KpiCard label="Sessions totales" value={stats.total ?? 0} />
          <KpiCard label="En cours" value={stats.in_progress ?? 0} />
          <KpiCard label="Terminées" value={stats.completed ?? 0} />
          <KpiCard label="Moy. tours / session" value={stats.avg_turns ?? '-'} />
          <KpiCard
            label="Duree moyenne"
            value={formatDuration(stats.avg_duration)}
          />
          <KpiCard
            label="Porte dominante"
            value={
              topDoor ? DOOR_LABELS[topDoor.door] || topDoor.door : '-'
            }
            sub={topDoor ? `${topDoor.count} sessions` : undefined}
          />
        </div>
      )}

      {stats?.avg_petals &&
        Object.values(stats.avg_petals).some((v) => v > 0) && (
          <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6">
              <FlowerSVG
                petals={scoresToPetals(stats.avg_petals)}
                size={240}
                animate
                showLabels
              />
              <div className="grid grid-cols-2 gap-x-6 gap-y-1.5 text-sm">
                {Object.entries(stats.avg_petals).map(([k, v]) => (
                  <div key={k} className="flex items-center gap-2">
                    <span className="text-xs text-slate-400 w-16">
                      {PETAL_LABELS[k] || k}
                    </span>
                    <div className="flex-1 h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden w-20">
                      <div
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 overflow-hidden">
        <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex flex-wrap gap-3 items-end">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors flex items-center gap-1.5 ${
                shadowOnly
                  ...
                  : 'bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-800 hover:bg-rose-100 dark:hover:bg-rose-950/50'
              }`}
            >
              🌑 Ombres détectées
            </button>
          ...
          <div>
            ...
            <input
              ...
              className="mt-1 px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-sm focus:outline-none"
            />
          </div>
          <div>
            <label className="text-[10px] text-slate-400 uppercase tracking-widest">
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      {s.door_suggested ? (
                        <span className="px-2 py-0.5 rounded-full bg-violet-100 dark:bg-violet-900 text-violet-600 dark:text-violet-300 text-xs font-medium">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1.5 items-center">
                        ...
                        {shadowBadge && (
                          <span
                            className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${shadowBadge.cls}`}
                            title={`${s.shadow_event_count ?? 0} détection(s)`}
                          >
                            {shadowBadge.icon} {shadowBadge.label}
                          </span>
                        )}
                      </div>
                    </td>
        ...
        {sessions && (sessions.pages ?? 1) > 1 && (
          <div className="p-4 flex gap-2 justify-center border-t border-slate-100 dark:border-slate-800">
            ...
            <button
              disabled={page >= (sessions.pages ?? 1)}
              onClick={() => setPage((p) => p + 1)}
              className="px-3 py-1 rounded-lg border text-sm disabled:opacity-40"
            >
              &rarr;
