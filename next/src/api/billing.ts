...
export const billingApi = {
  ...
  getUserRedemptions: (userId: number) => api.get(`/api/promo/user-redemptions?user_id=${userId}`),
  getUserUsage: (userId: number) =>
    api.get(`/api/admin/user-usage?user_id=${userId}`),
  adminAssignAccess: (data: Record<string, unknown>) => api.post('/api/promo/admin-assign', data),
  removeRedemption: (id: number) => api.post('/api/promo/remove-redemption', { id }),
  adminCreditUsage: (
    userId: number,
