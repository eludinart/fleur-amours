...
import {
  ...
  getLinkedInShareUrl,
  type SharePayload,
} from '@/utils/share-social'

type ShareSocialButtonsProps = {
  payload: SharePayload
  onCopyLink?: () => void
  /** Masquer certains réseaux (ex. ['linkedin']) */
  exclude?: ('twitter' | 'facebook' | 'whatsapp' | 'linkedin')[]
  ...
  return (
    <div className={`flex flex-col gap-2 ${className}`}>
      ...
      {buttons.map(({ key, href, icon }) => (
        ...
        >
          {icon}
          {variant === 'labels' && <span className="text-xs font-medium">{LABELS[key]}</span>}
        </a>
      ))}
      <button
        type="button"
